...
impl Visitor<Value> for JsonGenerator {
    ...
    fn visit_struct_decl(&mut self, struct_decl: &StructDecl) -> Value {
        ...
        json!({
            "type": "StructDecl",
            "line": struct_decl.node.line,
            "column": struct_decl.node.column,
    ...
    fn visit_var_decl(&mut self, var_decl: &VarDecl) -> Value {
        json!({
            "type": "VarDecl",
            "line": var_decl.node.line,
            "column": var_decl.node.column,
            "var_type": var_decl.var_type.to_string(),
            "name": var_decl.name,
            "initializer": var_decl.initializer.as_ref().map(|init| self.visit_expression(init))
        })
    }
    ...
    fn visit_block(&mut self, block: &BlockStmt) -> Value {
        ...
        json!({
            "type": "Block",
            "line": block.node.line,
            "column": block.node.column,
    ...
    fn visit_if_stmt(&mut self, if_stmt: &IfStmt) -> Value {
        json!({
            ...
            "column": if_stmt.node.column,
            "condition": self.visit_expression(&if_stmt.condition),
            "then_branch": self.visit_statement(&if_stmt.then_branch),
            "else_branch": if_stmt.else_branch.as_ref().map(|else_branch| self.visit_statement(else_branch))
        })
    }
    ...
    fn visit_while_stmt(&mut self, while_stmt: &WhileStmt) -> Value {
        json!({
            ...
            "column": while_stmt.node.column,
            "condition": self.visit_expression(&while_stmt.condition),
            "body": self.visit_statement(&while_stmt.body)
        })
    }
    ...
    fn visit_for_stmt(&mut self, for_stmt: &ForStmt) -> Value {
        json!({
            ...
            "column": for_stmt.node.column,
            "init": for_stmt.init.as_ref().map(|init| self.visit_statement(init)),
            "condition": for_stmt.condition.as_ref().map(|cond| self.visit_expression(cond)),
            "update": for_stmt.update.as_ref().map(|update| self.visit_expression(update)),
            "body": self.visit_statement(&for_stmt.body)
        })
    }
    ...
    fn visit_return_stmt(&mut self, return_stmt: &ReturnStmt) -> Value {
        json!({
            "type": "ReturnStmt",
            "line": return_stmt.node.line,
            "column": return_stmt.node.column,
            "value": return_stmt.value.as_ref().map(|value| self.visit_expression(value))
        })
    }
    ...
    fn visit_expr_stmt(&mut self, expr_stmt: &ExprStmt) -> Value {
        json!({
            "type": "ExprStmt",
            "line": expr_stmt.node.line,
            "column": expr_stmt.node.column,
            "expression": self.visit_expression(&expr_stmt.expr)
        })
    }
    ...
    fn visit_literal(&mut self, literal: &Literal) -> Value {
        ...
        json!({
            "type": "Literal",
            "line": literal.node.line,
            "column": literal.node.column,
            "value": value
        })
    }
    ...
    fn visit_identifier(&mut self, identifier: &IdentifierExpr) -> Value {
        json!({
            "type": "Identifier",
            "line": identifier.node.line,
            "column": identifier.node.column,
    ...
    fn visit_binary(&mut self, binary: &BinaryExpr) -> Value {
        json!({
            "type": "BinaryExpr",
            "line": binary.node.line,
            "column": binary.node.column,
            "operator": binary.operator.to_string(),
            "left": self.visit_expression(&binary.left),
            "right": self.visit_expression(&binary.right)
        })
    }
    ...
    fn visit_unary(&mut self, unary: &UnaryExpr) -> Value {
        json!({
            "type": "UnaryExpr",
            "line": unary.node.line,
            "column": unary.node.column,
            "operator": unary.operator.to_string(),
            "operand": self.visit_expression(&unary.operand)
        })
    }
    ...
    fn visit_assignment(&mut self, assignment: &AssignmentExpr) -> Value {
        json!({
            "type": "AssignmentExpr",
            "line": assignment.node.line,
            "column": assignment.node.column,
            "operator": assignment.operator.to_string(),
            "target": self.visit_expression(&assignment.target),
            "value": self.visit_expression(&assignment.value)
        })
    }
    ...
    fn visit_struct_access(&mut self, access: &StructAccessExpr) -> Value {
        json!({
            ...
            "object": self.visit_expression(&access.object),
            "field": access.field
        })
    }
    ...
    fn visit_grouped(&mut self, grouped: &GroupedExpr) -> Value {
        json!({
            "type": "GroupedExpr",
            "line": grouped.node.line,
            "column": grouped.node.column,
            "expression": self.visit_expression(&grouped.expr)
        })
    }
}
